...
public class CyclicRotation {
  ...
  public static int[] cyclicRotation (int[] array, int rotation) {
    int length = array.length;
    if(rotation > length && length > 0) {
      rotation = rotation % length;
    }
    if(rotation == 0 || length < 2) {
      return array;
    }
    int[] result = new int[length];
    for(int index = 0; index < length; index++) {
      int destinationIndex = (index + rotation) % length;
      result[destinationIndex] = array[index];
    }
    return result;
  }
